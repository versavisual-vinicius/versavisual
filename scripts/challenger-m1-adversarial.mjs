...
console.log("\x1b[1m\x1b[35m🛡️  CHALLENGER M1-1: ADVERSARIAL STRESS TEST SUITE\x1b[0m")
console.log("\x1b[1m\x1b[36m===========================================================================\x1b[0m")
...
assert(css.includes("--color-teal-400: #70909c"), "Token --color-teal-400 é #70909c")
...
for (const f of m1Files) {
  ...
  assert(
    !/bg-teal[^"']*text-ink/i.test(content) && !/text-ink[^"']*bg-teal/i.test(content),
    `Ausência de bg-teal com text-ink em ${f}`,
    `Violação encontrada em ${f}`
  )
  assert(
    !/bg-teal[^"']*text-navy/i.test(content) && !/text-navy[^"']*bg-teal/i.test(content),
...
assert(
  headerCode.includes("min-h-[44px]") &&
  headerCode.includes("min-w-[44px]"),
...
const footerLinks = footerCode.match(/className="[^"]*min-h-\[44px\][^"]*"/g) || []
assert(
...
// WhatsAppFloat
assert(
  waCode.includes("min-h-[44px]") &&
  waCode.includes("min-w-[44px]"),
...
assert(allFontsPresent, "Todas as 12 fontes self-hosted (.woff2) existem em public/fonts/")

assert(
  !indexHtml.includes("fonts.googleapis.com") && !indexHtml.includes("fonts.gstatic.com"),
...
// ============================================================================
console.log("\n" + "=".repeat(75))
console.log("\x1b[1m\x1b[36mRESUMO DA AUDITORIA ADVERSARIAL MARCO 1 (M1)\x1b[0m")
